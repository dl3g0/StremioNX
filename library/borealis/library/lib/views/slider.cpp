...
Slider::Slider()
{
    ...
    this->addGestureRecognizer(new PanGestureRecognizer([this](PanGestureStatus status, Sound* soundToPlay)
        {
        ...
        float progress     = (status.position.x - getX() - pointer->getWidth() / 2) / paddingWidth;

        setProgress(progress);

        if (status.state == GestureState::UNSURE)
            *soundToPlay = SOUND_FOCUS_CHANGE;
        else if (status.state == GestureState::END)
