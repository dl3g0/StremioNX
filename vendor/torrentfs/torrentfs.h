...
// Same, but `cancel` (may be NULL) is polled while announcing and fetching BEP 9
// metadata, so a teardown aborts the open instead of blocking for the full
// per-tracker timeout. Returns NULL with err "cancelado" once aborted.
torrentfs *torrentfs_open_file_ex(const char *source, const char *cache_path,
                                  int file_index, const volatile bool *cancel,
                                  char *err, size_t errlen);
...
// Absolute byte offset of the streamed file within the torrent, in bytes.
int64_t torrentfs_stream_offset(const torrentfs *tfs);
