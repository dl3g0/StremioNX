...
/*
 * Resolve the path a file will be stored at WITHOUT requiring it to exist yet
 * (the file is created later by storage_open_ex). Mirrors the open-time path
 * selection: original path first, long-path fallback when too long.
 */
int storage_resolve_file_path(const metainfo_t *mi, const char *outdir,
                              uint32_t file_index, char *out, size_t out_size);
